...
// 1. Replace Firestore imports with RTDB imports
content = content.replace(
  `import { collection, addDoc, onSnapshot, query, orderBy, serverTimestamp, Timestamp } from 'firebase/firestore';`,
  `import { ref, push, onValue, serverTimestamp } from 'firebase/database';`
);

// 2. Replace the old RsvpAndWishesSection with RTDB version
const newSection = `
...
const regexReplace = /\/\/ 8\. RSVP \& Wishes \(Youtube-style Comments\)\nfunction RsvpAndWishesSection\(\) \{[\s\S]*?\/\/ 10\. Gift \/ Hadiah/m;
content = content.replace(regexReplace, newSection + '\n\n// 10. Gift / Hadiah');

fs.writeFileSync(appPath, content, 'utf8');
console.log('App.tsx updated for RTDB successfully.');
